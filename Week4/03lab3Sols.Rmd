...
The classification tree has 4 levels (including the leaves) and 7 nodes (3 standard nodes and 4 ofleaves).
...
A petal length of 1.9 or smaller on its own is enough to distinguish all instances of setosa from the rest of the instances. 
...
The confusion matrices obtained have row and column values transposed compared to the ones obtained with other functions.
...
OPTIONAL Exercise 9a (one possible solution for Iris using a for loop to minimise code)
```{r}
ctrl <- trainControl(method = "repeatedcv", 
                     number =4, 
                     repeats = 3,
